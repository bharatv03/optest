...
foreach($currentQue as $forKey=>$valueQue)
{
    ...
    <div id="<?php echo 'for-'.$forKey.'-q-'.$valueQue['queNo']; ?>">
        ...
        <div id="q-<?php echo $valueQue[0]['final_id']; ?>" class="<?php echo $valueQue[0]['question_for']; ?> english que"><?php echo $valueQue[0]['e_sub_question']; ?></div>
        ...
        <br />
        <button id="mart-answer">Mark for Review & Next</button>
        <button id="clear-answer">Clear Response</button>
        <button id="save-answer">Save & Next</button>
    ...
    else
    {
        ...
        <div id="q-<?php echo $valueQue[0]['final_id']; ?>" class="<?php echo $valueQue[0]['question_for']; ?> hindi que"><?php echo $valueQue[0]['h_sub_question']; ?></div>
...
<div class="right-exam">
    ...
    <div class="down-box">
        ...
        <?php
        //print_r($this->Session->read('paperData'));die;
        foreach($this->Session->read('paperData')[$this->Session->read('currentTab')] as $keyPage=> $queVal)
        {
            ?>
            <button <?php if(isset($queVal['visited'])){ ?> class="<?php echo 'visited-'.$queVal['visited']; ?>" <?php } ?>><?php echo ($keyPage + 1); ?></button> 
            <?php
        }
        ?>
    </div>
